...
type CrossoverAlgorithm<'a> = ('a array) -> ('a array) -> ('a array * 'a array)
...
let cycle : CrossoverAlgorithm<_> = fun p1 p2 ->
    ...
    let rec loop i =
        if o1 |> Array.contains p2.[i] then
            ...
            let j = p1 |> Array.findIndex (fun x -> x = p2.[i])
            o1.[j] <- p1.[j]
            o2.[j] <- p2.[j]
            loop j
    ...
    let missingGenes =
        p1
        |> Array.indexed
        |> Array.filter (fun (_, g) -> o1 |> Array.contains g |> not)
        |> Array.map fst
...
let cycle2 : CrossoverAlgorithm<_> = fun p1 p2 ->
    ...
    let step1 i p1Val (p1: _ array) (p2: _ array) =
        o1.[i] <- p1Val
        let pos = p1 |> Array.findIndex (fun x -> x = o1.[i])
        let posInOne = p1 |> Array.findIndex (fun x -> x = p2.[pos])
        o2.[i] <- p2.[posInOne]
        p1 |> Array.findIndex (fun x -> x = o2.[i])
    
    let selectedBit = step1 0 p2.[0] p1 p2
    
    let rec loop exitCondition (p1: _ array) (p2: _ array) selected i =
        if exitCondition() || i = o1.Length then
            i
        else
            o1.[i] <- p2.[selected]
            let posInOne = p1 |> Array.findIndex (fun x -> x = o1.[i])
            let valInTwo = p2.[posInOne] //5
            let posInTwo = p1 |> Array.findIndex (fun x -> x = valInTwo)
            o2.[i] <- p2.[posInTwo]
            let nextSelect = p1 |> Array.findIndex (fun x -> x = o2.[i])
    ...
    if finalI <> o1.Length then
        let missingP1 = p1 |> Array.filter (fun x -> o1 |> Array.contains x |> not)
        let missingP2 = p2 |> Array.filter (fun x -> o1 |> Array.contains x |> not)
...
let primitive (rand: Random) : CrossoverAlgorithm<_> = fun (p1: _ array) (p2: _ array) ->
    ...
    let aCities = aIndexes |> Array.map (fun i -> p1.[i])
    let bCities = p2 |> Array.except aCities
    let res = aCities |> Array.append bCities
    (res, res)
...
let partiallyMatched a b : CrossoverAlgorithm<_> = fun p1 p2 ->
    ...
    let rec findValue i' (p1': _ array) (p2': _ array) (o: _[]) =
        let idxOpt = o |> Array.tryFindValueIndex p1'.[i']
        match idxOpt with
        | ValueSome _ ->
            let idxIn2 = p2' |> Array.findIndex (fun x -> x = p1'.[i'])
